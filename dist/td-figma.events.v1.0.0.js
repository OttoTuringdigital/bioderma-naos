/* TD Figma Events Bundle v1.0.0 | config + runtime */
/* TD Figma Events Config v1.0.0
 * ------------------------------------------------------------
  ...
  window.TDFigmaEventsConfig = {
    ...
    rules: [
       ...
       * ====================================================== */
      {
        ...
        selector: '.nav-tab .nav-tab-link, .slider-nav-ul-recommend a, .slider-nav-ul-recommend button, .panel-heading-recommend a, .panel-heading-recommend button, [data-tdpp-v1-recommend-tab]',
...
/* TD Figma Events Runtime v1.0.0 | ES5 syntax */
(function (window, document) {
  ...
  function isFloatingSidebar(element) {
    ...
    var level = 0;
    var image;
    if (closest(element, '.tdfn-v1-root, #tdfn-v1-desktop-portal, .tdff-v1-root, .ns-tool-box, #hsearch, .nav-search-box, .search-box', null)) { return false; }
    image = String(element.tagName || '').toLowerCase() === 'img' ? element : safeQuery('img[alt]', element);
    if (!image || !trim(image.getAttribute('alt'))) { return false; }
    while (current && current !== document.body && level < 7) {
      try { style = window.getComputedStyle(current); } catch (error) { style = null; }
      if (style && (style.position === 'fixed' || style.position === 'sticky')) {
        rect = current.getBoundingClientRect();
        if (rect.width > 0 && rect.height > 0 && rect.left >= window.innerWidth * 0.65) { return true; }
      }
      current = current.parentElement;
      level += 1;
    }
  ...
  function init() {
    ...
    firePageRules();
    setupScroll();
    if (state.clickRules.length) { document.addEventListener('click', handleClick, false); }
    setupImpressions();
