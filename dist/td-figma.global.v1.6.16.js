/* TD_Figma_All_Pages_Core_Style_GTM_v1.8.9.html */
(function(){window.TDFigmaStyleReady=window.TDFigmaStyleReady||{};window.TDFigmaStyleReady.allPages="1.8.9";}());
...
/* TD_Figma_All_Pages_Navigation_GTM_v5.6.0.html */
(function () {
  'use strict';

  var job = {
    key: 'td-figma-navigation-v560',
    ready: function () {
...
children: mapSimpleLinks(item.children || [])
});
}
return output;
}
...
function safeFeaturedNumber(value, fallback, minimum, maximum) {
...
function mapFeaturedLinks(items) {
var output = [];
var i;
var item;
for (i = 0; i < (items || []).length; i += 1) {
...
function mapFeaturedSections(content) {
var output = [];
var sections;
var section;
var sectionType;
...
section = sections[i] || {};
sectionType = section.sectionType === 'links' ? 'links' : 'cards';
items = Array.isArray(section.items) ? section.items : [];
output.push({
id: section.id || ('featured-section-' + (i + 1)),
heading: section.heading || section.label || '',
type: sectionType,
weight: safeFeaturedNumber(section.desktopWeight, 1, 0.25, 12),
columns: Math.round(safeFeaturedNumber(section.desktopColumns, sectionType === 'cards' ? 1 : 1, 1, 6)),
visible: section.visible !== false,
items: sectionType === 'links' ? mapFeaturedLinks(items) : mapCards(items)
});
}
return output;
}
...
content = content || {};
primaryCards = Array.isArray(content.primaryCards) ?
content.primaryCards : (content.primaryCard ? [content.primaryCard] : []);
secondaryCards = Array.isArray(content.secondaryCards) ?
content.secondaryCards.slice(0) : [];
oldAdditional = Array.isArray(content.additionalLinks) ? content.additionalLinks : [];
for (i = 0; i < oldAdditional.length; i += 1) {
secondaryCards.push({
title: oldAdditional[i].label || '',
linkUrl: oldAdditional[i].linkUrl || ''
});
}

return [
{
id: 'sitewide-campaigns',
heading: '全館活動',
type: 'cards',
weight: 1,
columns: 1,
visible: true,
items: mapCards(primaryCards.slice(0, 1))
},
{
id: 'hot-campaigns',
heading: '熱銷活動',
...
columns: 4,
visible: true,
items: mapCards(secondaryCards.slice(0, 4))
},
{
id: 'recommended',
heading: '推薦主打',
...
columns: 1,
visible: true,
items: mapFeaturedLinks(primaryCards.slice(1))
},
{
id: 'official-picks',
heading: '官網嚴選',
...
columns: 1,
visible: true,
items: mapFeaturedLinks(secondaryCards.slice(4))
}
];
}
...
var KNOWLEDGE_CONTENT = getMenuContent('knowledge');
var CLASSROOM_CONTENT = getMenuContent('classroom');
var ABOUT_CONTENT = getMenuContent('about');
var FEATURED_SECTIONS = mapFeaturedSections(FEATURED_CONTENT);
var CONFIG = {
desktopMaxWidth:
...
var NEEDS_LINKS = mapSimpleLinks(
NEEDS_CONTENT.links || []
);
var LEGACY_CLASSROOM_LINKS = mapCardLinks(
CLASSROOM_CONTENT.articleCards || []
);
var hasNestedClassroom = false;
var needsIndex;
for (needsIndex = 0; needsIndex < NEEDS_LINKS.length; needsIndex += 1) {
if (
NEEDS_LINKS[needsIndex].id === 'classroom' ||
NEEDS_LINKS[needsIndex].label === '貝膚小教室'
) {
hasNestedClassroom = true;
if (!NEEDS_LINKS[needsIndex].id) {
NEEDS_LINKS[needsIndex].id = 'classroom';
}
if (!NEEDS_LINKS[needsIndex].children.length && LEGACY_CLASSROOM_LINKS.length) {
NEEDS_LINKS[needsIndex].children = LEGACY_CLASSROOM_LINKS;
}
}
}
/* 向下相容舊 Dataset：即使貝膚小教室仍是第一層，也在 Runtime 自動搬進依肌膚需求。 */
if (!hasNestedClassroom && LEGACY_CLASSROOM_LINKS.length) {
NEEDS_LINKS.unshift({
id: 'classroom',
label: '貝膚小教室',
href: '',
children: LEGACY_CLASSROOM_LINKS
});
}
var DATA = {
menus: MENU_ITEMS.filter(function (item) {
return (item.id || '') !== 'classroom';
}).map(function (item) {
return {
id: item.id || '',
label: item.label || '',
...
href: item.linkUrl || '',
isAllProducts: item.isAllProducts === true || item.id === 'all-products' || item.displayName === '全系列產品'
};
}),
knowledge: mapSimpleLinks(
KNOWLEDGE_CONTENT.links || []
),
medical: mapCards(
...
portal: null,
activeDesktopId: '',
activeDesktopChildId: '',
activeDesktopKnowledgeSecondId: '',
activeDesktopKnowledgeThirdId: '',
...
function getNestedItem(items, childId) {
var i;
var item;
for (i = 0; i < (items || []).length; i += 1) {
item = items[i];
if (item && (item.id === childId || (!item.id && item.label === childId))) {
return item;
}
}
return null;
}
function renderNeedsDesktop() {
var activeChild = getNestedItem(DATA.needs, state.activeDesktopChildId);
var html = [
'<div class="tdfn-v1-panel tdfn-v1-needs-panel' + (activeChild ? ' has-third-level' : '') + '">',
'<div class="tdfn-v1-needs-secondary">'
];
var i;
var item;
var label;
var childId;
for (i = 0; i < DATA.needs.length; i += 1) {
item = DATA.needs[i];
if (item.visible === false) { continue; }
label = getItemLabel(item);
if (item.children && item.children.length) {
childId = item.id || label;
html.push(
'<button class="tdfn-v1-compact-link tdfn-v1-needs-expand' +
(state.activeDesktopChildId === childId ? ' is-active' : '') +
'" type="button" data-tdfn-d-child-open="' + escapeHtml(childId) + '">' +
'<span>' + escapeHtml(label) + '</span>' + chevronRight() +
'</button>'
);
} else {
html.push(
'<a class="tdfn-v1-compact-link" href="' + escapeHtml(getItemHref(item)) + '" ' +
'data-tdfn-link data-menu="needs" data-label="' + escapeHtml(label) + '">' +
escapeHtml(label) + '</a>'
);
}
}
...
if (activeChild) {
html.push(
'<div class="tdfn-v1-needs-third">',
'<div class="tdfn-v1-needs-third-head">',
'<strong>' + escapeHtml(activeChild.label) + '</strong>',
'<button type="button" class="tdfn-v1-needs-third-close" data-tdfn-d-child-close aria-label="關閉第三層">',
'<i class="ico ico-close" aria-hidden="true"></i>',
'</button>',
'</div>',
'<div class="tdfn-v1-needs-third-list">'
);
for (i = 0; i < activeChild.children.length; i += 1) {
item = activeChild.children[i];
label = getItemLabel(item);
html.push(
'<a class="tdfn-v1-needs-third-link" href="' + escapeHtml(getItemHref(item)) + '" ' +
'data-tdfn-link data-menu="needs-classroom" data-label="' + escapeHtml(label) + '">' +
escapeHtml(label) + '</a>'
);
}
html.push('</div></div>');
...
var i;
for (i = 0; i < (items || []).length; i += 1) {
if (items[i] && items[i].visible !== false) { output.push(items[i]); }
}
return output;
}
function renderKnowledgeDesktop() {
var secondItems = visibleChildItems(DATA.knowledge);
var activeSecond = getNestedItem(secondItems, state.activeDesktopKnowledgeSecondId);
var thirdItems = activeSecond ? visibleChildItems(activeSecond.children) : [];
var activeThird = activeSecond ? getNestedItem(thirdItems, state.activeDesktopKnowledgeThirdId) : null;
var html = [
'<div class="tdfn-v1-panel tdfn-v1-knowledge-panel' + (activeSecond ? ' has-third' : '') + (activeThird ? ' has-fourth' : '') + '">',
'<div class="tdfn-v1-knowledge-column tdfn-v1-knowledge-second">'
];
var i;
var item;
var label;
var itemId;
for (i = 0; i < secondItems.length; i += 1) {
item = secondItems[i];
label = getItemLabel(item);
itemId = item.id || label;
html.push(
'<button class="tdfn-v1-compact-link tdfn-v1-knowledge-expand' + (state.activeDesktopKnowledgeSecondId === itemId ? ' is-active' : '') + '" type="button" data-tdfn-d-knowledge-second="' + escapeHtml(itemId) + '">' +
'<span>' + escapeHtml(label) + '</span>' + chevronRight() + '</button>'
);
}
html.push('</div>');
if (activeSecond) {
html.push('<div class="tdfn-v1-knowledge-column tdfn-v1-knowledge-third">');
for (i = 0; i < thirdItems.length; i += 1) {
item = thirdItems[i];
label = getItemLabel(item);
itemId = item.id || label;
if (item.children && visibleChildItems(item.children).length) {
html.push(
'<button class="tdfn-v1-compact-link tdfn-v1-knowledge-expand' + (state.activeDesktopKnowledgeThirdId === itemId ? ' is-active' : '') + '" type="button" data-tdfn-d-knowledge-third="' + escapeHtml(itemId) + '">' +
'<span>' + escapeHtml(label) + '</span>' + chevronRight() + '</button>'
);
} else if (getItemHref(item)) {
html.push('<a class="tdfn-v1-compact-link" href="' + escapeHtml(getItemHref(item)) + '" data-tdfn-link data-menu="knowledge" data-label="' + escapeHtml(label) + '">' + escapeHtml(label) + '</a>');
} else {
html.push('<span class="tdfn-v1-compact-link is-disabled">' + escapeHtml(label) + '</span>');
}
}
html.push('</div>');
}
if (activeThird) {
html.push(
'<div class="tdfn-v1-knowledge-column tdfn-v1-knowledge-fourth">',
'<div class="tdfn-v1-knowledge-fourth-head"><strong>' + escapeHtml(activeThird.label) + '</strong><button type="button" class="tdfn-v1-knowledge-close" data-tdfn-d-knowledge-close aria-label="關閉第四層"><i class="ico ico-close" aria-hidden="true"></i></button></div>',
'<div class="tdfn-v1-knowledge-article-list">'
);
thirdItems = visibleChildItems(activeThird.children);
for (i = 0; i < thirdItems.length; i += 1) {
item = thirdItems[i];
label = getItemLabel(item);
if (!getItemHref(item)) { continue; }
html.push('<a class="tdfn-v1-knowledge-article-link" href="' + escapeHtml(getItemHref(item)) + '" data-tdfn-link data-menu="knowledge-article" data-label="' + escapeHtml(label) + '">' + escapeHtml(label) + '</a>');
}
html.push('</div></div>');
}
html.push('</div>');
...
section = sections[i] || {};
items = section.items || [];
if (section.visible === false || !items.length) {
continue;
}
sectionClass = 'tdfn-v1-featured-section is-' +
(section.type === 'links' ? 'links' : 'cards');
style = '--tdfn-v1-featured-weight:' +
String(section.weight || 1) + ';--tdfn-v1-featured-columns:' +
String(section.columns || 1) + ';';
html.push(
'<section class="' + sectionClass + '" style="' + style + '" data-tdfn-featured-section="' +
escapeHtml(section.id || '') + '">'
);
if (section.heading) {
html.push(
'<h3 class="tdfn-v1-section-title">' +
...
html.push('<div class="tdfn-v1-featured-card-grid">');
for (j = 0; j < items.length; j += 1) {
html.push(
renderFeaturedCard(
items[j],
'tdfn-v1-featured-section-card'
)
);
}
html.push('</div>');
...
html.push('<a class="'+cls+' tdfn-v1-series-all-products" href="'+escapeHtml(getItemHref(item))+'" data-tdfn-link data-menu="series" data-label="'+escapeHtml(item.zh)+'"><span class="'+nameCls+'">'+escapeHtml(item.zh)+'</span></a>');
continue;
}
...
if (id === 'needs') {
return renderNeedsDesktop();
}
if (id === 'knowledge') {
return renderKnowledgeDesktop();
}
if (id === 'types') {
...
if (menuId === 'needs') {
width = state.activeDesktopChildId ? 410 : (menu.width || 155);
left = rect.left;
} else if (menuId === 'knowledge') {
width = state.activeDesktopKnowledgeThirdId ? 565 : (state.activeDesktopKnowledgeSecondId ? 325 : (menu.width || 155));
left = rect.left;
} else {
...
if (state.activeDesktopId !== menuId || menuId !== 'needs') {
state.activeDesktopChildId = '';
}
if (state.activeDesktopId !== menuId || menuId !== 'knowledge') {
state.activeDesktopKnowledgeSecondId = '';
state.activeDesktopKnowledgeThirdId = '';
}
clearDesktopTimer();
portal = ensurePortal();
...
function renderMobileHeading(title, parentId) {
var backAttr = parentId ?
'data-tdfn-m-back-parent="' + escapeHtml(parentId) + '" aria-label="返回上一層"' :
'data-tdfn-m-close aria-label="返回主選單"';
return '<div class="tdfn-v1-m-heading"><button class="tdfn-v1-m-sub-close" type="button" '+backAttr+'>'+backArrow()+'</button><h2 class="tdfn-v1-m-title">'+escapeHtml(title)+'</h2></div>';
}
...
if (item.children && visibleChildItems(item.children).length) {
html.push(
'<button class="tdfn-v1-m-sub-link tdfn-v1-m-sub-expand" type="button" ' +
'data-tdfn-m-open="' + escapeHtml(id + '::' + (item.id || label)) + '">' +
'<span>' + escapeHtml(label) + '</span>' + chevronRight() +
'</button>'
);
} else {
...
function getNestedPathItem(source, pathParts) {
var currentItems = source || [];
var current = null;
var i;
for (i = 0; i < pathParts.length; i += 1) {
current = getNestedItem(currentItems, pathParts[i]);
if (!current) { return null; }
currentItems = current.children || [];
}
return current;
}
function renderMobileNestedPath(parentId, pathParts) {
var source = parentId === 'needs' ? DATA.needs : (parentId === 'knowledge' ? DATA.knowledge : []);
var item = getNestedPathItem(source, pathParts);
var html;
var i;
var child;
var label;
var currentId = parentId + '::' + pathParts.join('::');
var parentPath = pathParts.slice(0, -1);
var backId = parentPath.length ? parentId + '::' + parentPath.join('::') : parentId;
var children;
if (!item) {
return renderMobileSimple(parentId, getMenu(parentId) ? getMenu(parentId).label : '', source);
}
children = visibleChildItems(item.children || []);
html = ['<div class="tdfn-v1-m-content">' + renderMobileHeading(item.label, backId) + '<div class="tdfn-v1-m-sublist">'];
for (i = 0; i < children.length; i += 1) {
child = children[i];
label = getItemLabel(child);
if (child.children && visibleChildItems(child.children).length) {
html.push('<button class="tdfn-v1-m-sub-link tdfn-v1-m-sub-expand" type="button" data-tdfn-m-open="' + escapeHtml(currentId + '::' + (child.id || label)) + '"><span>' + escapeHtml(label) + '</span>' + chevronRight() + '</button>');
} else if (getItemHref(child)) {
html.push('<a class="tdfn-v1-m-sub-link" href="' + escapeHtml(getItemHref(child)) + '" data-tdfn-link data-menu="' + escapeHtml(parentId) + '" data-label="' + escapeHtml(label) + '">' + escapeHtml(label) + '</a>');
} else {
html.push('<span class="tdfn-v1-m-sub-link is-disabled">' + escapeHtml(label) + '</span>');
}
}
html.push('</div></div>');
return html.join('');
}
function renderMobileFeaturedSingleCard(item) {
if (!item || !item.title) {
return '';
}
return (
...
function renderMobileFeaturedCarousel(section) {
...
html.push(
'<section class="tdfn-v1-m-block tdfn-v1-m-featured-section" data-tdfn-featured-section="' +
escapeHtml(section.id || '') + '">'
);
if (section.heading) {
html.push(
'<h3 class="tdfn-v1-m-section-title">' +
escapeHtml(section.heading) +
'</h3>'
);
}
...
html.push(renderMobileFeaturedSingleCard(items[0]));
} else {
html.push(renderMobileFeaturedCarousel(section));
}
html.push('</section>');
}
html.push('</div>');
...
var nestedParts = String(id || '').split('::');
if (nestedParts.length >= 2) {
return renderMobileNestedPath(nestedParts[0], nestedParts.slice(1));
}
if (id === 'featured') {
...
if (id === 'knowledge') {
return renderMobileSimple(id, '保養知識庫', DATA.knowledge);
}
...
var backParent = closest(event.target, '[data-tdfn-m-back-parent]', root);
...
event.preventDefault();
event.stopPropagation();
openMobilePanel(backParent.getAttribute('data-tdfn-m-back-parent'));
return;
}
if (close) {
event.preventDefault();
...
portal.addEventListener('click', function (event) {
var openChild = closest(event.target, '[data-tdfn-d-child-open]', portal);
var closeChild = closest(event.target, '[data-tdfn-d-child-close]', portal);
var knowledgeSecond = closest(event.target, '[data-tdfn-d-knowledge-second]', portal);
var knowledgeThird = closest(event.target, '[data-tdfn-d-knowledge-third]', portal);
var knowledgeClose = closest(event.target, '[data-tdfn-d-knowledge-close]', portal);
var trigger;
if (openChild) {
event.preventDefault();
event.stopPropagation();
state.activeDesktopChildId = openChild.getAttribute('data-tdfn-d-child-open') || '';
portal.innerHTML = renderNeedsDesktop();
trigger = state.desktopRoot ? state.desktopRoot.querySelector('[data-tdfn-d-trigger="needs"]') : null;
positionDesktopPortal('needs', trigger);
track('td_nav_menu_open', { td_nav_device: 'desktop', td_nav_menu: 'needs-third', td_nav_label: state.activeDesktopChildId });
return;
}
if (closeChild) {
event.preventDefault();
...
event.preventDefault();
event.stopPropagation();
state.activeDesktopKnowledgeSecondId = knowledgeSecond.getAttribute('data-tdfn-d-knowledge-second') || '';
state.activeDesktopKnowledgeThirdId = '';
portal.innerHTML = renderKnowledgeDesktop();
trigger = state.desktopRoot ? state.desktopRoot.querySelector('[data-tdfn-d-trigger="knowledge"]') : null;
positionDesktopPortal('knowledge', trigger);
track('td_nav_menu_open', { td_nav_device: 'desktop', td_nav_menu: 'knowledge-third', td_nav_label: state.activeDesktopKnowledgeSecondId });
return;
}
if (knowledgeThird) {
event.preventDefault();
event.stopPropagation();
state.activeDesktopKnowledgeThirdId = knowledgeThird.getAttribute('data-tdfn-d-knowledge-third') || '';
portal.innerHTML = renderKnowledgeDesktop();
trigger = state.desktopRoot ? state.desktopRoot.querySelector('[data-tdfn-d-trigger="knowledge"]') : null;
positionDesktopPortal('knowledge', trigger);
track('td_nav_menu_open', { td_nav_device: 'desktop', td_nav_menu: 'knowledge-fourth', td_nav_label: state.activeDesktopKnowledgeThirdId });
return;
}
if (knowledgeClose) {
event.preventDefault();
...
/* TD_Figma_All_Pages_Footer_GTM_v3.0.2.html */
(function () {
  'use strict';

  var job = {
    key: 'td-figma-footer-v302',
    ready: function () {
      return !!(
        window.TDFigmaData &&
        window.TDFigmaData.allPages &&
    ...
    start: function () {
      (function () {
        ...
        function contactMarkup(mobile) {
          ...
          html.push(
            '<div class="tdff-v1-service">' +
            '<div class="tdff-v1-service-title">' +
            '服務時間' +
            '</div>' +
            '<p class="tdff-v1-service-description">' +
            '如有任何疑問，歡迎登入後點選右下角紅色對話框留言' +
            '</p>' +
            '</div>'
          );
